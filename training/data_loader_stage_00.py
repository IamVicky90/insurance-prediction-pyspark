...
class Data_Loader:
    def __init__(self, log_collection_name:str, path:str, is_log_enabled:str):
        ...
        self.logger = get_log_object_for_training(log_collection_name,is_log_enabled)
        self.path = path
    ...
    def load_data_from_path(self):
        ...
        try:
            ...
            df= self.ss.read.csv(self.path)
            self.logger.log(f"Sucessfully Load the data from path {self.path}")
            return df
        except Exception:
...
if __name__ == "__main__":
    ...
    args.add_argument("--data_source")
    parsed_args=args.parse_args()
    logger=get_log_object_for_training(log_collection_name)
    data_loader = Data_Loader(
        log_collection_name, parsed_args.data_source, parsed_args.is_log_enabled)
    df=data_loader.load_data_from_path()
