...
from utility import read_params, get_log_object_for_prediction
import argparse
import sys
import ast
...
class Data_Loader:
    def __init__(self, is_log_enabled: bool,log_collection_name: str = log_collection_name):
        ...
        self.logger = get_log_object_for_prediction(
...
if __name__ == "__main__":
    ...
    args.add_argument(
        "--data_source", default=["data/Prediction_Batch_files/HealthPrem_26092020_131534.csv"])
    parsed_args=args.parse_args()
    if (parsed_args.is_log_enabled) == str:
        parsed_args.is_log_enabled = ast.literal_eval(
            parsed_args.is_log_enabled.title())
    logger = get_log_object_for_prediction(log_collection_name)
