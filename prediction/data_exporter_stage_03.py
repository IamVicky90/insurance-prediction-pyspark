    ...
    def convert_mongo_data_to_master_csv(self, db_name, collection_name, path, master_csv_filename):
        ...
        try:
            ...
            df = self.__db.get_data_from_mongo_db(db_name, collection_name)[
                utility.read_params()["data"]["prediction_main_columns"]]
            self.__logger.log("dataframe extracted")
            df.to_csv(os.path.join(path,master_csv_filename),index=False)
        except Exception:
            error = Export_Data_Exception("Error in module {0} class {1} method {2}".format(
                export_data.__module__, export_data.__class__.__name__, self.convert_mongo_data_to_master_csv.__name__), sys)
            raise error
...
def validate_data_exporter(master_csv_path, master_csv_filename, log_collection_name, config, is_log_enabled):
    ...
    db_name = config['database_for_prediction_data']['database_name']
    collection_name = config['database_for_prediction_data']['collection_name']
